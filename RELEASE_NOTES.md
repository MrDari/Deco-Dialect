# Notas de versión (release notes) — Deco Dialect

> Copia/pega el bloque del idioma correspondiente en Play Console al crear la versión:
> **Producción/Pruebas → Crear nueva versión → "Notas de la versión"**.
> Van entre las etiquetas `<es-ES>…</es-ES>` y `<en-US>…</en-US>` (máx. 500 caracteres por idioma).
> Usa el idioma/idiomas que tengas activos en la ficha.

---

## v1.7 — versionCode 9 (actual)

**Español (es-ES):**
```
¡Nuevos modos! Contrarreloj (gana tiempo acertando), Solo doradas y Difícil, además del Clásico. Elige tema (Naturaleza, Mundo, Cultura…), encadena rachas, siente la vibración en cada acierto y celebra con confeti y récords. Reloj visual de anillo y pantalla de configuración renovada. ¡Gracias por jugar!
```

**English (en-US):**
```
New modes! Blitz (earn time by scoring), Golden only and Hard, alongside Classic. Pick a theme (Nature, World, Culture…), chain streaks, feel the haptics on every hit, and celebrate with confetti and records. Ring timer and redesigned setup screen. Thanks for playing!
```

---

## Historial (por si necesitas reusar texto)

### v1.6 — versionCode 8
- ¡Ahora gratis! 15 categorías y partidas de 2 equipos y 3 rondas; versión completa con pago único (90 categorías, 5 equipos, 20 rondas). Diseño renovado.

### v1.5 — versionCode 7
- Arreglo: el modal de compra se cierra solo al confirmarse (era asíncrono).

### v1.4 — versionCode 6
- Modelo gratis + desbloqueo único (compra `unlock_full`).

### v1.3 — versionCode 5
- Arreglo de la barra de navegación de Android: la interfaz ya no queda bajo los
  botones del sistema en ningún móvil.

### v1.2 — versionCode 4
- Botón "Saltar" estable y placa de ronda más discreta.
